...
namespace RawCMS.Library.Core.Helpers
{
    public class ReflectionManager
    {
        ...
        public Type GetTypeByName(string typeName, bool partialMatch, bool includeDeps)
        {
            return GetTypeByName(this.AssemblyScope, typeName, partialMatch, includeDeps);
        }

        /// <returns></returns>
        public Type GetTypeByName(List<Assembly> assemblies, string typeName, bool partialMatch, bool includeDeps)
        {
            List<Assembly> searchAssembly = new List<Assembly>();
            searchAssembly.AddRange(assemblies);
            if (includeDeps)
            {
                assemblies.ForEach(x =>
                {
                    searchAssembly.AddRange(x.GetReferencedAssemblies().Select(Assembly.Load).ToList());
                });
            }

            foreach (Assembly assembly in searchAssembly)
            {
                if (assembly.FullName.Contains("Newton"))
                {
                    var c = typeof(JObject).Assembly;
                }
                // _logger.LogDebug("loading from" + assembly.FullName);
                Type[] types = assembly.GetTypes();
                foreach (Type type in types)
