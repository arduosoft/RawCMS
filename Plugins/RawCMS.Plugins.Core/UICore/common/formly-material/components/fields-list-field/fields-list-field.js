    ...
    res({
        computed: {
            cmpCollName: function () {
                ...
                if (
                    ...
                    vuexStore.state.core.relationMetadata.formId !==
                    this.field._meta_.formId
                ) {
                    return undefined;
                }

                return optionalChain(
                    () => vuexStore.state.core.relationMetadata.collectionName
                );
            }
        },
        extends: baseDef,
        methods: {
            updateAvailableFields: async function (collName) {
                ...
                const fields = optionalChain(() => entity.FieldSettings, {
                    fallbackValue: []
                }).map(x => x.Name);
