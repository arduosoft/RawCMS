...
const _RawCmsDataTableDef = async () => {
    const tpl = await RawCMS.loadComponentTpl(
        "/app/common/shared/components/data-table/data-table.tpl.html"
    );
    ...
    return {
        ...
        data: function () {
            return {
                apiService: this.apiBasePath
                    ? new BaseCrudService({ basePath: this.apiBasePath })
                    : null,
                currentItem: {},
                currentPage: 1,
                dataHeaders: [],
        ...
        methods: {
            ...
            goTo: function (item) {
                if (!this.detailRouteName) {
                    console.warn(
                        '"detailRouteName" prop was not defined, cannot go to detail view.'
                    );
                    return;
                }
            ...
            getDataHeaders: async function () {
                throw new Error(
                    `Please provide an implementation for ${this.getDataHeaders.name}`
                );
            },
