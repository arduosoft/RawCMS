...
import { randomString } from "/app/utils/random.utils.js";
import { validationService } from "/app/modules/core/services/validation.service.js";
...
const _FieldEditDef = async () => {
  const tpl = await RawCMS.loadComponentTpl(
    "/app/modules/core/components/field-edit/field-edit.tpl.html"
  );
  ...
  return {
    computed: {
      ...
      isAllValid: function() {
        return (
          this.isValid &&
          optionalChain(() => this.optionsFormState.$valid, {
            fallbackValue: true
          })
        );
      },
      nameRules: function() {
        return [
          val =>
            val !== undefined && val !== ""
              ? true
              : this.$t("core.validation.required")
        ];
      }
    },
    created: function() {
    ...
    methods: {
      close: function(isOk) {
        this.$emit(_fieldEditEvents.closed, {
          isOk: isOk,
