...
const _LambdaDetailsDef = async () => {
    ...
    const tpl = await RawCMS.loadComponentTpl(
        "/app/modules/core/components/lambda-details/lambda-details.tpl.html"
    );
        ...
        data: function () {
            return {
                nameRules: [
                    ...
                    v =>
                        (v && v.length <= 10) ||
                        this.$t("core.lambdas.details.nameTooLongMsg")
                ],
                pathRules: [
                    v => !!v || this.$t("core.lambdas.details.requiredPathMsg"),
                    v =>
                        (v && v.length <= 20) ||
                        this.$t("core.lambdas.details.pathTooLongMsg")
                ],
                customMonacoOptions: {
                    language: "javascript",
        ...
        methods: {
            ...
            resizeCustomMonaco: function () {
                ...
                const newHeight =
                    ...
                    this.$refs.customContainer.computedStyleMap().get("padding-top")
                        .value -
                    this.$refs.customContainer.computedStyleMap().get("padding-bottom")
                        .value -
                    this.$refs.firstRow.getBoundingClientRect().height -
                    this.$refs.monacoContainer.computedStyleMap().get("padding-top")
                        .value -
                    this.$refs.monacoContainer.computedStyleMap().get("padding-bottom")
                        .value;
