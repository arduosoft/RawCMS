  ...
  return {
    ...
    methods: {
      deleteConfirmMsg(item) {
        return this.$t("core.configuration.deleteConfirmMsgTpl", {
          name: item.plugin_name
        });
      },
      deleteSuccessMsg(item) {
        return this.$t("core.configuration.deleteSuccessMsgTpl", {
          name: item.plugin_name
        });
      },
      deleteErrorMsg(item) {
        return this.$t("core.configuration.deleteErrorMsgTpl", {
          name: item.plugin_name
        });
      }
    },
...
const _ConfigurationListDef = async () => {
  ...
  const tpl = await RawCMS.loadComponentTpl(
    "/app/modules/core/components/configuration-list/configuration-list.tpl.html"
  );
